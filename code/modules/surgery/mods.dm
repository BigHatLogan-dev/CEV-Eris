...
/datum/surgery_step/attach_mod/can_use(mob/living/user, obj/item/organ/internal/organ, obj/item/mod)
	...
	if(limb && C)
		var/organ_size_delta = ((organ.specific_organ_size + C.modifications[ORGAN_SPECIFIC_SIZE_BASE]) * (1 + C.modifications[ORGAN_SPECIFIC_SIZE_MULT]) + C.modifications[ORGAN_SPECIFIC_SIZE_MOD]) - organ.specific_organ_size
